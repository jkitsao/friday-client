...
function addFile(target, file) {
  ...
  empty.classNameList.add("hidden");
...
function dropHandler(ev) {
  ...
  for (const file of ev.dataTransfer.files) {
    addFile(gallery, file);
    overlay.classNameList.remove("draggedover");
    counter = 0;
  }
}
...
function dragEnterHandler(e) {
  ...
  ++counter && overlay.classNameList.add("draggedover");
}

function dragLeaveHandler(e) {
  1 > --counter && overlay.classNameList.remove("draggedover");
}
...
gallery.onclick = ({ target }) => {
  if (target.classNameList.contains("delete")) {
    const ou = target.dataset.target;
    document.getElementById(ou).remove(ou);
    gallery.children.length === 1 && empty.classNameList.remove("hidden");
    delete FILES[ou];
  }
};
...
document.getElementById("cancel").onclick = () => {
  ...
  FILES = {};
  empty.classNameList.remove("hidden");
